            ...
            return AsyncInfo.Run<IBuffer, uint>(async (token, progress) =>
            {
                ...
                for (var i = pieceStart; i <= pieceEnd; i++)
                {
                    ...
                    while (!_torrentFile.BitField[i])
                    {
                        ...
                        _manager.SlidingPicker.HighPrioritySetSize = Math.Max(TorrentStreamManager.DefaultPrepareCount, i - pieceEnd);
                        // TODO instead of looping, use event
                        await Task.Delay(500, token);
                    }
                }

                var notDownloaded = _torrentFile.BitField.FirstFalse();
                if (notDownloaded > -1)
                {
